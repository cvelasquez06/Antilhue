...
for registers in ${Array[*]}; do
	...
	dirDestination=$dirRootBackup$year"/"$monthNow"/"$year$monthNow$dayNow"_"$nameDayNow"_"$numWeek
	dirBackupTotalDestination=$dirRootTotal$year"/"$monthNow
	...
	start=$(date +'%s')
	run=$(rsync -av --delete-before --force $maxSizeFiles --prune-empty-dirs $directory $dirDestination$directory $excludeBackup --compare-dest=$dirBackupTotalDestination$directory)
	echo $run
	time_elapsed=$(($(date +'%s') - $start))
	echo "->"$time_elapsed "second "
	...
	du=$(du -bsh $dirBackupCompressedUpload$name".tar.gz")
	echo "->final size compress "$du
done
echo "->compress all files tar.gz generateds on "$dirBackupCompressed" backup local"
echo "->destination files "$dirCompressedFinal$year$monthNow$dayNow".tar.gz"
start=$(date +'%s')
GZIP=-9 tar --remove-files -zcf $dirCompressedFinal$year$monthNow$dayNow".tar.gz" $dirBackupCompressed*".tar.gz" > /dev/null 2>&1
time_elapsed=$(($(date +'%s') - $start))
echo "->compress success "
echo "->"$time_elapsed "second "
echo "->validate files compress integrity "$dirCompressedFinal$year$monthNow$dayNow".tar.gz backup local"
start=$(date +'%s')
if ! tar -tf dirCompressedFinal$year$monthNow$dayNow".tar.gz" > /dev/null 2>&1; then
	echo "->Archivo malformado "$dirCompressedFinal$year$monthNow$dayNow".tar.gz"
else
	echo "->integrity success "
fi
echo "->"$time_elapsed "second "
echo "->compress all files tar.gz generateds on "$dirBackupCompressedUpload" to upload S3"
echo "->destination files "$dirCompressedFinalUpload$year$monthNow$dayNow".tar.gz"
start=$(date +'%s')
GZIP=-9 tar --remove-files -zcf $dirCompressedFinalUpload$year$monthNow$dayNow".tar.gz" $dirBackupCompressedUpload*".tar.gz" > /dev/null 2>&1
time_elapsed=$(($(date +'%s') - $start))
echo "->compress success "
echo "->"$time_elapsed "second "


echo "->validate files compress integrity "$dirCompressedFinalUpload$year$monthNow$dayNow".tar.gz to upload S3"
start=$(date +'%s')
upload=1
if ! tar -tf $dirCompressedFinalUpload$year$monthNow$dayNow".tar.gz" > /dev/null 2>&1; then
	echo "->Archivo malformado "$dirCompressedFinalUpload$year$monthNow$dayNow".tar.gz"
	upload=0
else
	echo "->integrity success "
fi
...
if [ $upload -eq 1 ];then
	...
	start=$(date +'%s')
	s3cmd put $dirCompressedFinalUpload$year$monthNow$dayNow".tar.gz" s3://antilhue-backup/backup/backups_incrementales/
	time_elapsed=$(($(date +'%s') - $start))
	echo "->"$time_elapsed "second "
fi
